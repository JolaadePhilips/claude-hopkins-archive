(function enhanceSwipeFile(){
  ...
  function normalise(ad){
    ...
    return {
      ...ad,
      context: ad.context || (ad.metadataConfidence === 'auto'
      ...
      whyItWorks: ad.whyItWorks || (ad.metadataConfidence === 'auto'
        ? 'This record has not yet received a hand-written analysis. Use the original scan first; detailed swipe-file commentary is only added after the execution has been checked manually.'
        : [ad.hook, ad.proof].filter(Boolean).join(' ')),
      publication: ad.publication || 'Original publication not yet verified',
      creditLine: ad.creditLine || (ad.sourceLabel
      ...
      sourceStatus: ad.sourceStatus || (ad.metadataConfidence === 'auto' ? 'Exact scan provenance · metadata provisional' : 'Hand-verified record')
    };
  }
  ...
  window.openDetail = function(ad){
    ...
    const verificationLinks = (a.verificationSources || []).map(s => `<a class="source-link" href="${esc(s.url)}" target="_blank" rel="noreferrer">↗ ${esc(s.label)}</a>`).join('');

    document.querySelector('#detailInner').innerHTML = `
      <div class="detail-grid">
        <div class="detail-image"><img src="${esc(a.image)}" alt="${esc(a.brand)} advertisement: ${esc(a.headline)}"></div>
